...
require 'test/unit'
class TestWD3 < Test::Unit::TestCase
  include TestWDSetup

	# User logged in
  # Request to warning diff for directory page
  ...
  # Request to warning diff for directory page
	...
	def test_wd_st_002
		 ...
		 open_warning_diff_directory
		 				assert(is_element_present($warning_diff_xpath["warning_listing_button"]))
		 @selenium.click $warning_diff_xpath["warning_listing_button"]
		 sleep SLEEP_TIME*9
		 assert(is_text_present($warning_diff["warning_listing_text"]))
		 logout
	end
  ...
  # Request to warning diff for directory page
    ...
    old_table = get_xpath_count($warning_diff_xpath["row"])
		@selenium.select "others", "Rule Number"
  ...
  # Request to warning diff for directory page
	...
	# Move to "Summary of Warning (for file)" page
	def test_wd_st_013
    printf "\n+ Test 013"
    assert true
  end
  ...
  def test_wd_st_018
	...
	# User logged in
  # Request to warning diff for directory page
	# Click on "Summary of Warning (for directory)" link
	# Back to this page
	def test_wd_st_019
    printf "\n+ Test 019"
    open_warning_diff_directory
